...
func Webhook(event *models.GhPushEvent) (*models.WebhookResponse, error) {
	if event == nil {
	...
	if len(paths) == 0 {
		return &models.WebhookResponse{Owner: owner, Repo: repo, HeadSHA: head, FilesCount: 0, Files: nil}, nil
	}

	files := make([]models.FetchedFile, 0, len(paths))
	for p := range paths {
		content, err := fetchRepoFile(owner, repo, head, p, event.Repository.Private)
		if err != nil || content == "" {
			continue
		}
		sum := sha256.Sum256([]byte(content))
		ff := models.FetchedFile{
			Path:          p,
			RefSHA:        head,
			Content:       content,
...
func collectYamlPaths(e *models.GhPushEvent) map[string]struct{} {
...
func fetchRepoFile(owner, repo, ref, path string, isPrivate bool) (string, error) {
...
func pushJob(j models.Job) (*models.EntryView, error) {
	d, err := time.ParseDuration(j.Spec.Timeout)
	if err != nil {
		return nil, errors.New("duration format is invalid e.g 30s")
	}
	s := &models.ScheduleRequest{
