...
try {
    ...
    $dataDir = "$env:LOCALAPPDATA\monocron"
    New-Item -ItemType Directory -Path $dataDir -Force | Out-Null

    Write-Host "$App installed to $InstallDir"
    Write-Host "Join the runner before starting:"
    Write-Host "  monocronctl runner join <token>"
    Write-Host "Then run:"
    Write-Host "  `$env:MONOCRON_CONTROLLER_URL = 'https://ctrl.example.com'"
